...
WHY THIS EXISTS: THE DESTRUCTIVE RESET (user decision — pilot-phase, dev-local conversation
data only; nothing is migrated). The unified-chat rebuild replaces the SPA-driven parts-shaped
transcript with NATIVE pydantic-ai batches, and the two shapes share nothing worth carrying:

  * DELETEs every conversation row (messages cascade; `attachments.conversation_id` is
    `ON DELETE SET NULL`, so orphaned uploads age into the never-sent reclaimer's sweep);
  * DROPs the legacy `messages` table + `message_role` enum, RECREATEs `messages` in the
    native-batch shape (`entry_kind`/`visibility`/`mode`, JSONB `payload`, `meta`, `seq`);
  * adds sticky `conversations.mode` (default `plan`), drops the legacy `code` snapshot
    column (truth now lives in `app_registry.current_code` + build snapshots).

Apps, snapshots, projects, usage, approvals, users are UNTOUCHED. `downgrade` is
structure-only — the legacy shape returns EMPTY, a one-way door for data both ways, by
design. Hand-finalized rather than autogenerated.
"""

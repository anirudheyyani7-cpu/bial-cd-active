...
`force-end`; everything else answers a non-leaking 404. The mutating POSTs carry the
reusable `RequireCsrf` dependency; the `status` GET and the GET-SSE progress feed
...
async def start_build(
    ...
    with build_coordination_or_503():
        try:
            ...
            # An attached file could not be materialized (missing bytes, a magic-byte
            # mismatch, a deck, over the per-file text ceiling). FAIL the start naming the file
            # rather than building as if the file weren't there — silently ignoring it is the
            # exact bug this guards against. Nothing was allocated (the resolution runs before
            # the lock and the sandbox), so there is no compensation to run. `str(exc)` is the
            # service's own user-facing copy, never an internal detail.
            raise AppApiError(
...
async def report_client_error(
    ...
    202, not 200: nothing has happened yet when this returns. The report is parked for the next
    `selfheal.verify` to collect, where it makes the verdict not-green — the whole user-visible
    consequence is that the completion claim does not appear, because the report's own text
    goes to the AGENT and to nobody else. `recorded: false` is still a 202: the one thing that
    can be refused here is a report the store had no room for, which is a fact about volume the
    caller deserves to be told rather than a client error to raise."""
    ...
    # `unhandledrejection` / `console.*` captures to the framing portal, which validates the
    # frame's origin and forwards what it caught. This route is the receiving end.
    #

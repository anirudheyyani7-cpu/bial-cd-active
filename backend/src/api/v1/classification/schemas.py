...
THE PER-QUESTION PROJECTION IS DELIBERATELY TWO FIELDS (`verdict`, `reason`). The stored `verdicts`
document also carries `agreed_with_scan`, `downgraded_from_yes` and the compact scan block — the
administrator's dispute presentation, which is the administrator's concern and not the citizen's —
and the sibling `evidence` document carries cited locations. Building the wire model field-by-field
from exactly two keys is what makes "nothing from evidence, for any verdict" structural rather than
a filter someone has to remember when the stored shape grows a field.
"""
...
# (`service._FLOOR_UNANSWERED_REASON`), owned here as this surface's own copy: it maps
...
class QuestionReview(CamelModel):
    ...
    from `no`: the review answers only where it has evidence, and an unanswered
...
class ReviewAnswers(CamelModel):
    ...
    def all_unanswered(cls) -> ReviewAnswers:
        """This presentation of a failure with no Tier A floor: six unanswered
...
class ClassificationReviewResponse(CamelModel):
    ...
    the dialog filters by the stamp it asked for, so a second tab's newer review can never

"""The `user_limits` table — sparse per-user limit overrides.

Absent row (or a NULL column) → the global default (`settings.DAILY_TOKEN_LIMIT` for the
daily cap). Mirrors the Express sparse per-user `limits` object: each override is nullable
and NULL means "use the default". The daily gate reads `daily_token_limit`; the context
limits are SPA per-conversation guardrails (not part of the daily gate) carried here so the
admin limits-patch writes the whole cohesive `limits` unit without a later schema change.

...
class SessionManager:
    ...
    async def _stop_the_held_session(
        ...
        *,
        ...
        """The stop itself, with NO DB SESSION — which is what lets it run detached: the
        caller resolves the app row (a user-scoped read) and hands the id over, so the task
        `request_stop_of_active_work` starts cannot outlive a request's `get_db` and read from
        a closed connection. TWO KINDS OF LIVE, one door: `_start_locked` registers a build
        session carrying a `run_build` task; `ensure_sandbox` registers a Write turn's
        workspace with no task at all, since the work runs in the turn engine instead. Either
        way an agent is working from the container's point of view, so the caller should not
        have to know which — the branch is here."""

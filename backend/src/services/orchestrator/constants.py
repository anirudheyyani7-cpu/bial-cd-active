...
Distinct from the daily token quota (per-user, DB) and the self-heal budget."""

RUN_TOKEN_BUDGET = 750_000
"""Hard SPEND ceiling on a single build turn, the third of three bounds on one loop.
...
real production incident, where one calculator build booked 956k of a 1M daily cap on 68
tokens of real fresh input.

SIZED AGAINST A REAL TRACE, not chosen for roundness: a demo build once spent ~938k
...
# --- harness-driven commands + timeouts ---------------------------------------
# The model now HAS an exec tool — `run_command`, a general shell over the same exec transport
# (the vibe-coding pivot). These two are the harness's OWN between-run verify invocation
...
errors, so `tsc --noEmit` is what the harness reads between runs. `next build` is NOT run
...
"""Wall-clock cap for EVERY OTHER model-driven `run_command`.
...
"""How often the DECOUPLED early readiness watcher polls `/dev/status`. It frames the
...
"""Truncation cap on a SUCCEEDING command's output. Summarise a success, dump
...
"""How many truncated outputs a turn holds for `fetch_output_slice` at once. The ring is
...
"""How many distinct commands one turn remembers for the repeat-run counter. A ceiling, not
...
how THIS loop is shaped, not a per-deployment knob.
...
"""Path segments the model never views — bounds context, not a threat surface."""
...
def is_write_allowed(path: str) -> bool:
    """Fail-closed write gate for `write_file` / `edit_file` / `insert_lines`, applied
    ...
    guard + demoted `appuser` are the actual containment."""
...
def is_read_ignored(path: str) -> bool:
    """True when the model may not `read_file` this path: a build-irrelevant heavy tree

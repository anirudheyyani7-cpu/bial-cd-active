...
_MUTATING_POSTS = [
    ...
    # U28 retired `lock/acquire` / `lock/renew` / `lock/release` / `heartbeat`, and U33 took the
    # last one standing: `lock/force-end` had no caller left on any surface once the block
    # banner's Force-end button went, so THERE ARE NO LOCK OPS IN THIS TABLE AT ALL. The kill
    # switch a citizen actually reaches is `projects/{project_id}/stop-active-build`, below.
...
def test_mutating_posts_matrix_matches_the_route_table() -> None:
    ...
    lost the dependency or was deleted — exactly the `test_csrf.py` half of the U28 and U33
    retirements that this file's own comments describe doing by hand.

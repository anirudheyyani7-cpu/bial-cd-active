...
branch on two kinds of live: a build session carrying a `run_build` task it cancelled itself, and
a Write turn's workspace it asked the turn engine to settle. Only the second exists now, so every
test here drives a REAL turn on a REAL `TurnEngine` (`_HoldsItsOwnUnwind` + the `_fresh_engine`
fixture) rather than a fake brain the manager could cancel directly. The subject is unchanged —
the three states, the ask/answer split, one stop per project — and it is now reached the one way

"""BRAIN persists its transcript per step (`entry_kind='step'` rows).
...
def _threaded_orchestrator(
    ...
    """A BuildOrchestrator whose run-context provider carries the conversation thread."""
    app_id = uuid.uuid4()
...
async def test_three_step_build_persists_incremental_batches(
    ...
    """A multi-model-step FunctionModel build persists incremental batches in
    order — one `[request, response]` pair per step (returns ride in the next step's request).

    THE LAST BATCH IS THE TOOL ANSWERS, NOT A THIRD PAIR. A passing `declare_done` ends
...
async def test_secrets_never_land_in_step_rows(db_session, billing_factory, sink) -> None:
    """A DSN travelling through tool args is stored redacted —

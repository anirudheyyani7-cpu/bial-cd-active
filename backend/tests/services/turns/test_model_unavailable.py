...
def test_the_signature_is_the_class_chain_the_status_and_the_provider_type_never_the_text() -> (
    ...
    assert re.fullmatch(
        ...
        r" at=tests\.services\.turns\.test_model_unavailable:\w+:\d+",
        signature,
    ), signature
...
def test_the_signature_survives_a_cycle_and_is_bounded() -> None:
    ...
    assert _error_signature(first) == "RuntimeError <- ValueError"
    ...
    for _ in range(10):
        ...
        deep = wrapper
    assert _error_signature(deep).count(" <- ") == 4
...
def test_the_provider_type_is_kept_only_as_a_short_lowercase_token(token: str) -> None:
    ...
    assert _error_signature(APIStatusError("detail")) == "APIStatusError status=529"

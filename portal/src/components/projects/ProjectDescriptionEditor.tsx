/**
 * The project's description surface: a read-only view with an Edit button opening a pop-up
 * editor (claude.ai project-instructions pattern) — authored by hand, or generated by the model
 * from the project's app code.
 *
 * Two behaviours are load-bearing, not polish: GENERATE SAVES FIRST — `…/description:generate`
 * revises the *stored* description, so a dirty (unsaved) field is `patchProject`-ed before
 * generating, or the generate would silently overwrite the typed edit; and EVERY FAILURE LEAVES
 * THE FIELD UNTOUCHED — the textarea only changes on a successful save/generate or by typing,
 * never an optimistic clear/replace. The model call is billed and takes seconds, so the whole
 * modal disables and shows a running note for its duration.
 */

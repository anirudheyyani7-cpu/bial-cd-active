...
└── template/              # the golden Next.js app (App Router + TS + Tailwind v4 + shadcn/ui)
    ...
    ├── components/bial/error-capture.tsx  # window.onerror/unhandledrejection/console capture
...
## Stack — latest-stable-then-pinned

Resolved to the newest stable at authoring and pinned into `package.json` + a real
...
## Data access — Drizzle owns the app's schema
...
> **Which build is authoritative depends on the subscription.** For org/prod the image is built
> and deployed from a **Windows VM** via `az acr build` → ACR `bialgenaicr01` → Azure App Service /
> ACA, so a green macOS/Linux `docker build` says nothing about the Windows/CRLF path and the
> cross-platform guards below are what stand between us and a repeat. **On the current free-trial
> subscription that path is unavailable** — ACR Tasks / `az acr build` are blocked — so the shipped
> artifact is produced by `docker buildx build --platform linux/amd64 --push` from macOS/Linux
...
# Run it. SUPERVISOR_TOKEN is required (fail-fast). BIAL_PORTAL_ORIGIN enables the frame header.
...
# Prove the framing header is on the next dev response and XFO is absent:

...
# else — SUPERVISOR_TOKEN, and any injected *_DSN / *_URL / *_PASSWORD the table below does not
# name — is DENIED by default. A suffix denylist is fail-OPEN: it can't anticipate what the
# platform injects (`IDENTITY_HEADER` matches no `_TOKEN/_SECRET/_KEY` suffix; a `*_DSN`
# connection string sails straight through), so an allowlist is the only safe scrub for an
# untrusted child. `IDENTITY_HEADER` is now admitted BY NAME through `_INJECTED_ENV` and redacted
# as a secret — which is exactly why it must be named there rather than left to a suffix rule.
...
#
# NOT ALL OF THESE ARE `BIAL_*` ANY MORE, AND THE TWO THAT ARE NOT ARE AZURE'S OWN. The moment a
# user-assigned managed identity is attached to this container app, Azure injects
# `IDENTITY_ENDPOINT` and `IDENTITY_HEADER` into the SUPERVISOR's environment — that pair IS the
...
_INJECTED_ENV: tuple[InjectedEnvVar, ...] = (
    ...
    # --- the connector data plane -------------------------------------------------------------
    # Two values, and NEITHER is a bearer credential: a URL and a client id are LABELS — holding
    # them grants nothing. The credential is the managed identity itself, minted by Azure inside
    # this container and impossible to copy out. This is the first capability on the platform
    # that injects no secret at all.
    #
    # THE NAMES ARE GENERATED BY THE BACKEND FROM THE CONNECTOR'S KEY and hardcoded here, because
    # `sandbox/` sits outside the search that keeps the connector's name out of `backend/src/`.
    # The two are produced independently, so they are asserted to agree for every registry entry
    # — see `backend/tests/services/build_sessions/test_appconnector_env.py`. They fail CLOSED
    # when they disagree: the backend sets a name this table does not carry, and the child simply
    # never sees it.
    InjectedEnvVar(
        "BIAL_DICE_URL",
    ...
    InjectedEnvVar(
        "IDENTITY_ENDPOINT",
        "Azure's token endpoint for the attached managed identity (a URL, not a credential)",
        False,
    ),
    InjectedEnvVar(
        "IDENTITY_HEADER",
        "the bearer for that token endpoint (secret — never printed)",
        True,
    ),
)
# The child-env allowlist: exactly the injected names, carried through the fail-closed scrub.
# NOT `_BIAL_*` any more — two of the names are Azure's, and calling this tuple after a prefix it
# no longer holds is how a reader ends up "tidying" the two rows that make the credential work.
...
def _child_env(extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    from the parent, then the names in `_INJECTED_ENV`, then the caller's `extra`. Denied by
    default: `SUPERVISOR_TOKEN`, and any `*_DSN` / `*_URL` / `*_PASSWORD` this table does not
    name — none of which a suffix denylist would have caught.

    `IDENTITY_ENDPOINT` / `IDENTITY_HEADER` USED TO BE THE HEADLINE DENIAL HERE AND ARE NOW
    ADMITTED ON PURPOSE. They are Azure's managed-identity token endpoint and its bearer, and the
    generated app needs them to read the connector's data lake as the identity attached to this
    container. The header is marked secret, so it is redacted from `/exec`, `/dev/logs` and
    `/files` exactly as the SAS and the database URL are. See the `_INJECTED_ENV` docblock.
    """
    env: dict[str, str] = {
        k: v
    ...
    # Carry the injected names through the scrub (ACA and Azure inject them into the parent env).
